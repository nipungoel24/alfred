---
type: module
generated: true
language: typescript
layer: frontend
qualified_name: frontend.src.features.overview.OverviewPage.test
source: frontend/src/features/overview/OverviewPage.test.tsx
status: active
tags: [module, frontend]
---

# frontend.src.features.overview.OverviewPage.test

> Auto-generated from source code. Do not manually edit this file; update source code or generator instead.

## Source

`frontend/src/features/overview/OverviewPage.test.tsx`

## Imports

- `vitest` â† describe, it, expect, vi, beforeEach
- `@testing-library/react` â† render, screen, fireEvent, waitFor
- `@tanstack/react-query` â† QueryClient, QueryClientProvider
- `./OverviewPage` â† OverviewPage
